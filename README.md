...
`verify-dice-rolls.mjs`, `verify-death-saves.mjs`, `verify-concentration.mjs`, and
`verify-vision-data-model.mjs` instead go
...
the app itself talks to Supabase; the dice-rolls/death-saves/concentration trio also drive
the roll Route Handler over real
...
`verify-action-economy.mjs`, `verify-opportunity-attacks.mjs`, and
`verify-vision-rendering.mjs` go one step further and
...
prompt's live landing and take/decline flow, and per-player vision masking recomputing
live in an open room — the latter five scripts also start
