...
"no tenant identifier provided" against a pooled connection. `verify-conditions.mjs` and
`verify-dice-rolls.mjs` instead go through `@supabase/supabase-js` (service-role client for
setup, real signed-in clients for the actual RLS/RPC checks) — equally valid for exercising
policies end to end, and closer to how the app itself talks to Supabase; the latter also drives
the roll Route Handler over real HTTP with signed-in session cookies (needs `yarn dev` running).
`verify-dice-ui.mjs` goes one step further and drives real Playwright browsers against the dev
server, for the parts only a live UI can exercise (the sheet's advantage toggle, the combat
panel's Roll-initiative buttons, live sync landing in an actually-open Game Room).
