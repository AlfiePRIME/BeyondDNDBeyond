...
export interface PresenceChannel {
  /** Broadcasts `payload` under `event` to every other subscriber of this channel's topic. */
  ...
  onPresenceChange(handler: (members: PresenceMember[]) => void): () => void;
  getPresentMembers(): PresenceMember[];
  /**
...
// channel. A caller that leaves and immediately rejoins the same topic
...
/**
 * Joins one Realtime channel for `topic` — presence plus a typed broadcast
 * pub/sub. This is the shared core behind the topic-scoped wrappers
 * (`joinCampaignChannel`, `joinLobbyChannel`); features should call one of
 * those rather than passing a raw topic string here, so topic naming stays
...
export function joinChannel(
  ...
  const presenceHandlers = new Set<(members: PresenceMember[]) => void>();
  const reconnectHandlers = new Set<() => void | Promise<void>>();
  const connectionStateHandlers = new Set<(state: ConnectionState) => void>();
  ...
  function getPresentMembers(): PresenceMember[] {
  ...
  // publish() awaits this so a caller can send right after joining
