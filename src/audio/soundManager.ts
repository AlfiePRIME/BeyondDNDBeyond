 ...
 * `hit_normal` is the one pool with real variety (>= 3 distinct files, per
 * SP5's "must genuinely vary" requirement); every other key has exactly one
 * canonical file. Array order has no meaning beyond variant selection below
 * — nothing depends on files[0] being "the" file for a pooled key.
 *
...
const SOUND_FILES: Record<SoundKey, string[]> = {
  dice_impact: ["/sounds/dice_impact.mp3"],
